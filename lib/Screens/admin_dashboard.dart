...
class _AdminDashboardState extends State<AdminDashboard>
  ...
  // REMOVED: Local _bookings getter - now using Firestore streams
  // REMOVED: getFilteredBookings - filtering now done inside StreamBuilder
                  ...
                  children: [
                    ...
                    Expanded(
                      child: IndexedStack(
                        index: _selectedTab,
                        children: [
                          _buildBookingsContent(),  // No longer needs filteredBookings parameter
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          debugPrint('Admin bookings stream error: ${snapshot.error}');
          return Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
        ...
        // Calculate counts from Firestore data
  ...
  // ANALYTICS SECTION WITH ALL CHARTS AND GRAPHS - REAL-TIME
  Widget _buildAnalyticsContent() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final confirmed = bookings.where((b) => b['status'] == 'Confirmed' || b['status'] == 'Approved').length;
    ...
    final dailyBookings = [
      ...
      bookings.where((b) => b['date'].contains('27')).length + 3,
      bookings.where((b) => b['date'].contains('26')).length + 1,
      bookings.where((b) => b['date'].contains('29')).length + 4,
      bookings.where((b) => b['date'].contains('25')).length + 2,
      1,
      0,
    ];
    final maxBookings = dailyBookings.reduce((a, b) => a > b ? a : b).toInt();
    final totalWeekly = dailyBookings.reduce((a, b) => a + b).toInt();
